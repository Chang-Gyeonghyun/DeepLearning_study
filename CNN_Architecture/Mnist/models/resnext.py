...
class Bottleneck(nn.Module):
  def __init__(self, in_channels, middle_channel, stride=1, down_sample=False):
    ...
    self.layer3 = nn.Sequential(
        nn.Conv2d(middle_channel, middle_channel*2, kernel_size=1,bias=False),
        nn.BatchNorm2d(middle_channel*2))

    if down_sample:
      self.down_sample = nn.Sequential(
                nn.Conv2d(in_channels, middle_channel*2, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(middle_channel*2),
            )
    else:
...
class ResNeXt(nn.Module):
    def __init__(self, block, layers, num_classes=10):
        ...
        # 레이어 블록 생성
        self.layer1 = self._make_layer(block, 128, layers[0], 1)
